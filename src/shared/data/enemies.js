import { preferWebpPath } from "../utils/assets.js";
...
export const ENEMIES = {
  old_man_judo: {
    ...
    ai: (turn) =>
      turn % 3 === 0
        ? { type: "block", value: 6 }
        : { type: "attack", value: turn % 2 === 0 ? 7 : 8 },
  },
  beastco: {
    id: "beastco",
    name: "Beastco",
  ...
  teej: {
    ...
    ai: (turn) =>
      turn % 3 === 0
        ? { type: "debuff", value: 2 }
        : { type: "attack", value: turn % 2 === 0 ? 12 : 14 },
  ...
  david: {
    ...
    onDebuff: (ctx) => {
      ...
      ctx.log("David schedules another meeting! Lose 1 energy next turn.");
    },
  },
  dax: {
    id: "dax",
  ...
  dhh: {
    ...
    onBlock: (ctx) => {
      ...
      ctx.enemy.hp = Math.min(ctx.enemy.maxHp, ctx.enemy.hp + heal);
      ctx.log(
        `DHH refactors the codebase with Ruby on Rails, healing ${heal} HP!`,
      );
    },
  },
};
