export const CARDS = {
  ...
  macro: {
    ...
    cost: 1,
    type: "skill",
    text: "Replay last card for free (once/fight).",
    art: "Monk_7.png",
    oncePerFight: true,
    effect: (ctx) => {
  ...
  segfault: {
    ...
    art: "Monk_8.png",
    effect: (ctx) => {
      ctx.deal(ctx.enemy, ctx.scalarFromWeak(7));
      ctx.draw(1);
    },
  },
  ...
  skill_issue: {
    ...
    cost: 1,
    type: "skill",
    text: "Gain 6 Block. If enemy intends attack → apply Weak(1).",
    art: "Monk_9.png",
    effect: (ctx) => {
      ctx.player.block += 6;
      if (ctx.intentIsAttack()) ctx.applyWeak(ctx.enemy, 1);
    },
  },
  ...
  dark_mode: {
    ...
    cost: 2,
    type: "attack",
    text: "Deal 20. End your turn.",
    art: "Monk_11.png",
    effect: (ctx) => {
      ctx.deal(ctx.enemy, ctx.scalarFromWeak(20));
      ctx.forceEndTurn();
    },
  },
  ...
  just_one_game: {
    ...
    cost: 1,
    type: "power",
    text: "Skip this turn. Next turn +2 Energy.",
    art: "Monk_18.png",
    effect: (ctx) => {
      ctx.flags.skipThisTurn = true;
      ctx.flags.nextTurnEnergyBonus = (ctx.flags.nextTurnEnergyBonus || 0) + 2;
    },
  },
  ...
  colon_q: {
    ...
    cost: 2,
    type: "attack",
    text: "Deal 1 per card in discard.",
    art: "Monk_19.png",
    effect: (ctx) =>
      ctx.deal(ctx.enemy, ctx.scalarFromWeak(Math.max(0, ctx.discard.length))),
  },
  ...
  task_failed_successfully: {
    ...
    cost: 2,
    type: "attack",
    text: "Deal 8. If enemy has no Block, deal 4 more.",
    art: "Monk_12.png",
    effect: (ctx) => {
  ...
  recursion: {
    ...
    cost: 2,
    type: "attack",
    text: "Deal 5. If this kills enemy, play again.",
    art: "Monk_13.png",
    effect: (ctx) => {
  ...
  memory_leak: {
    ...
    cost: 0,
    type: "skill",
    text: "Gain 1 Energy. Next turn -1 Energy.",
    art: "Monk_23.png",
    effect: (ctx) => {
      ctx.player.energy += 1;
      ctx.flags.nextTurnEnergyPenalty =
        (ctx.flags.nextTurnEnergyPenalty || 0) + 1;
    },
  },
  ...
  code_review: {
    ...
    cost: 1,
    type: "skill",
    text: "Look at top 3 cards. Put 1 in hand, rest on bottom of deck.",
    art: "Monk_24.png",
    effect: (ctx) => {
      ...
      ctx.root._codeReviewCallback = (selectedIndex) => {
        ...
        topCards.forEach((card, i) => {
          const drawIndex = ctx.root.player.draw.findIndex(
            (id) => id === card.id,
          );
          if (drawIndex >= 0) {
  ...
  pair_programming: {
    ...
    cost: 2,
    type: "skill",
    text: "Double next card's effect.",
    art: "Monk_25.png",
  ...
  hotfix: {
    ...
    cost: 2,
    type: "attack",
    text: "Deal 10. Can only be played if HP < 50%.",
    art: "Monk_15.png",
    effect: (ctx) => {
  ...
  ligma: {
    ...
    cost: 0,
    type: "skill",
    text: "Unalive yourself with -69 hit points. Courtesy of Defysall.",
    art: "Monk_26.png",
    effect: (ctx) => {
  ...
  virgin: {
    ...
    cost: 1,
    type: "skill",
    text: "If enemy intends to attack, gain 8 Block.",
    art: "Monk_27.png",
    effect: (ctx) => {
  ...
  production_deploy: {
    ...
    cost: 2,
    type: "attack",
    text: "Deal 25. Lose 5 HP.",
    art: "Monk_16.png",
    effect: (ctx) => {
      ctx.deal(ctx.enemy, ctx.scalarFromWeak(25));
      ctx.player.hp = Math.max(1, ctx.player.hp - 5);
      ctx.log(
        "Production deployment succeeds, but at what cost to your health?",
      );
    },
  },
  ...
  sugar_crash: {
    ...
    cost: 1,
    type: "curse",
    text: "Unplayable. -1 Energy when drawn.",
    art: "Monk_28.png",
    effect: (ctx) => {
  ...
  stack_overflow: {
    ...
    cost: 1,
    type: "attack",
    text: "Deal damage equal to cards in hand.",
    art: "Monk_29.png",
    effect: (ctx) =>
      ctx.deal(ctx.enemy, ctx.scalarFromWeak(ctx.player.hand.length)),
  },
  ...
  ctrl_z: {
    ...
    cost: 1,
    type: "skill",
    text: "Return a random card from discard to hand.",
    art: "Monk_30.png",
    effect: (ctx) => {
      if (ctx.player.discard.length > 0) {
        const randomId =
          ctx.player.discard[
            Math.floor(Math.random() * ctx.player.discard.length)
          ];
  ...
  infinite_loop: {
    ...
    cost: 2,
    type: "skill",
    text: "Play the same card twice this turn. Exhaust.",
    art: "Monk_32.png",
    exhaust: true,
    effect: (ctx) => {
  ...
  npm_audit: {
    ...
    cost: 1,
    type: "skill",
    text: "Gain 3 Block per curse in deck.",
    art: "Monk_33.png",
    effect: (ctx) => {
      ...
      ctx.log(
        `npm audit found ${curseCount} vulnerabilities. Gain ${blockGain} Block.`,
      );
    },
  },
  ...
  git_push_force: {
    ...
    cost: 0,
    type: "attack",
    text: "Deal 15. Put random card from hand on top of draw pile.",
    art: "Monk_34.png",
    effect: (ctx) => {
      ctx.deal(ctx.enemy, ctx.scalarFromWeak(15));
      if (ctx.player.hand.length > 0) {
        // Check if there are any cards left in hand
        const randomCard =
          ctx.player.hand[Math.floor(Math.random() * ctx.player.hand.length)];
